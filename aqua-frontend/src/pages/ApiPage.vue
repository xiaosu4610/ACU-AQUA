...
/* API 文档：自旧版 #page-api（index.html 1804-2283 行）1:1 静态平移；
   data-copy-from 复制按钮 → CopyBtn（text 为等价纯文本代码，见下方常量） */
import CopyBtn from '@/components/CopyBtn.vue'
import AqIcon from '@/components/AqIcon.vue'

/* 复制文本常量：与页面 <code> 高亮内容一一对应的纯文本 */
...
/* 高级计量通道调用示例（tide/ 按量计费章节用） */
const codeTideCurl = `curl https://api.ltzy.top/v1/chat/completions \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer sk-你的密钥" \\
  -d '{"model": "tide/glm-5.3-flash", "messages": [{"role": "user", "content": "你好"}], "max_tokens": 2048}'`
...
<template>
  <section class="route-page">
    <div class="api-page">
      <h1><span class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg></span>API 文档</h1>
      <p class="intro">AQUA 网关完整兼容 OpenAI 协议，所有端点均可直接用 OpenAI SDK 调用。统一入口见「首页」，以下为各端点的详细说明、调用示例与错误码。<b>API Key 账号制：注册登录后，在个人控制台一键创建密钥（sk-****），注册免费、随时吊销重建</b>，模型列表请以「模型中心 · 模型列表」为准（实时同步）。</p>
      <div class="repo-banner">
        ...
        <span><b>AQUA · ACU 工程系列开源项目</b>——你正在看的这份页面源码全部开放，欢迎自部署与二开</span>
      ...
      <div class="api-card api-anchor">
        ...
        <h3>一次调用的完整旅程</h3>
        <pre><code>你的程序                          AQUA 网关                        AI 模型（后厨）
   │                                 │                                │
   │  ① 递单子：带密钥的 HTTP 请求     │                                │
   ...
   ▼</code></pre>
      ...
      <div class="api-card api-anchor">
        ...
        <ul class="tips">
          ...
          <li><b>密钥</b>：<router-link to="/login" style="color:var(--accent);">注册登录</router-link>后，在<router-link to="/console" style="color:var(--accent);">个人控制台</router-link>创建——形如 <code>sk-****</code></li>
        </ul>
...
client = OpenAI(
    api_key=<span class="str">"sk-你的密钥"</span>,                              <span class="tok"># 官网控制台创建</span>
...
<span class="kwd">print</span>(r.choices[<span class="tok">0</span>].message.content)                       <span class="tok"># 打印 AI 的回答</span></code></pre>
        ...
        <pre><CopyBtn class="pre-copy" :text="codeQuickJs" /><code id="code-quick-js"><span class="kwd">import</span> OpenAI <span class="kwd">from</span> <span class="str">"openai"</span>;

<span class="kwd">const</span> client = <span class="kwd">new</span> OpenAI({
  apiKey: <span class="str">"sk-你的密钥"</span>,                                <span class="tok">// 官网控制台创建</span>
...
console.log(r.choices[<span class="tok">0</span>].message.content);</code></pre>
  ...
  <span class="str">"usage"</span>: { ... }                 <span class="tok">// 用量统计（本站免费，仅供参考）</span>
}</code></pre>
        <h3>新手最容易踩的三个坑</h3>
        <ul class="tips">
      ...
      <div class="api-card api-anchor">
        ...
        <p class="desc">先把结论说清楚：<b>本站绝大多数模型完全免费，且会一直免费</b>。收费的只有<b>官方自营专线系列</b>（<code>aqua/</code> 按次计费加速通道，共 5 款；<code>tide/</code> 按量计费高级计量通道，共 24 款含文生图）。下面把免费、收费、怎么充值、错误怎么算账，一次讲明白。</p>
        <h3>哪些免费？——除它之外全部免费</h3>
        <ul class="tips">
        ...
        <h3>哪些收费？——aqua/ 专线系列（5 款）</h3>
        <p class="desc">官方自营<b>加速通道</b>（会员专属算力池，极致速度、高频场景优选）： <code>aqua/deepseek-v4-flash</code>、<code>aqua/glm-5.3-flash</code>、<code>aqua/deepseek-v4-pro</code>、<code>aqua/glm-5.2</code>、<code>aqua/glm-5.3</code>。调用方式和免费模型<b>完全一样</b>——同一个接口、同一把密钥、同一套 SDK，只是 <code>model</code> 字段换成它们：</p>
  ...
  -d <span class="str">'{"model": "aqua/glm-5.3-flash", "messages": [{"role": "user", "content": "你好"}]}'</span>   <span class="tok"># 只换了 model，其余一字不差</span></code></pre>
        <p class="desc">支持 <code>stream: true</code> 流式输出，效果与免费模型一致，对客户端完全透明。需先<b>注册登录</b>并使用<b>个人密钥</b>调用（会话令牌调用收费模型会被拒绝）。全线收费模型为<b>按次计费（正式价 ¥0.004~0.006/次）</b>——每次成功请求按模型单价扣一次，与生成长度无关。各模型实时单价见「模型中心」收费模型专区或 <code>/v1/models</code> 返回的 <code>price_micro</code> 字段。</p>
        <h3>还有一条线——tide/ 高级计量通道（24 款，按量计费）</h3>
        <p class="desc">官方自营<b>高级计量通道</b>（<code>tide/</code> 前缀）：覆盖全系列大杯旗舰对话模型与 <code>tide/qwen-image-2.0</code>、<code>tide/wan2.7-image</code> 两款文生图模型。<b>按量计费三段价</b>——输入 / 缓存命中 / 输出分开计价（元 / 百万 tokens），用多少付多少、无保底。同样是同一个接口、同一把密钥，只换 <code>model</code>：</p>
        <pre><CopyBtn class="pre-copy" :text="codeTideCurl" /><code id="code-tide-curl"><span class="kwd">curl</span> https://api.ltzy.top/v1/chat/completions \
  -H <span class="str">"Content-Type: application/json"</span> \
  -H <span class="str">"Authorization: Bearer sk-你的密钥"</span> \
  -d <span class="str">'{"model": "tide/glm-5.3-flash", "messages": [{"role": "user", "content": "你好"}], "max_tokens": 2048}'</span>   <span class="tok"># max_tokens 越小，单次预扣越低</span></code></pre>
        <p class="desc">文生图模型走 <code>/v1/images/generations</code>（OpenAI 兼容），<b>¥0.1/张</b>按张计费，返回站内直链图片 URL，<code>n</code> 参数控制张数。tide/ 全线<b>先付后用</b>：发起请求时按预估（输入 + <code>max_tokens</code> 输出上限）预扣，完成后<b>多退少补</b>；重复对话前缀会命中<b>缓存价</b>，输入成本大幅更低。各模型实时三段价见「模型中心」收费模型专区或 <code>/v1/models</code> 返回的 <code>in_price / cache_price / out_price</code> 字段。</p>
        <h3>收费政策与资金安全（四条铁律）</h3>
        <div class="md-table-wrap"><table class="md-table">
          <thead><tr><th>规则</th><th>具体内容</th><th>对你的意义</th></tr></thead>
          <tbody>
            <tr><td><b>按次计费</b></td><td>每次<b>成功</b>请求按所用模型单价扣一次：正式价 <b>¥0.004~0.006/次</b>。与生成长度无关。</td><td>价格公开透明，写一句话和写一千字同价</td></tr>
        ...
        <h3>如何获得余额</h3>
        <p class="desc"><b>在线充值即时到账</b>：登录后进入<router-link to="/console?view=topup" style="color:var(--accent);">个人控制台 · 余额充值</router-link>，支付宝 / 微信任一渠道支付，<b>支付金额 100% 全额到账</b>（渠道手续费由本站承担），到账立即可用。余额与消费在<router-link to="/console" style="color:var(--accent);">个人控制台</router-link>顶部实时可见。</p>
      ...
      <div class="api-card api-anchor">
        ...
        <p class="desc">AI 没有记忆。每次调用都是一场<b>全新的、没有前世记忆</b>的对话——所谓多轮对话，就是<b>你每次都把之前的完整聊天记录一起递回去</b>，像复读整本笔记本：</p>
        <pre><CopyBtn class="pre-copy" :text="codeMultiTurn" /><code id="code-multi-turn"><span class="tok"># 第二轮提问时，messages 里要带上第一轮的全部内容：</span>
messages = [
  {<span class="str">"role"</span>: <span class="str">"system"</span>,   <span class="str">"content"</span>: <span class="str">"你是一个耐心的编程老师"</span>},   <span class="tok"># 人设说明书</span>
  {<span class="str">"role"</span>: <span class="str">"user"</span>,     <span class="str">"content"</span>: <span class="str">"Python 的列表是什么？"</span>},       <span class="tok"># 第 1 问</span>
...
<span class="kwd">for</span> chunk <span class="kwd">in</span> stream:                      <span class="tok"># 每次循环 = 一小段</span>
    piece = chunk.choices[<span class="tok">0</span>].delta.content <span class="kwd">or</span> <span class="str">""</span>
    <span class="kwd">print</span>(piece, end=<span class="str">""</span>, flush=<span class="kwd">True</span>)     <span class="tok"># 逐段打印，肉眼可见的打字机</span></code></pre>
        <pre><CopyBtn class="pre-copy" :text="codeStreamJs" /><code id="code-stream-js"><span class="tok">// JS（浏览器原生 SSE，无需任何库）</span>
...
<span class="kwd">let</span> buf = <span class="str">""</span>, full = <span class="str">""</span>;
<span class="kwd">while</span> (<span class="kwd">true</span>) {
  <span class="kwd">const</span> { done, value } = <span class="kwd">await</span> reader.read();
  <span class="kwd">if</span> (done) <span class="kwd">break</span>;
  buf += dec.decode(value, { stream: <span class="kwd">true</span> });
  <span class="kwd">const</span> lines = buf.split(<span class="str">"\n"</span>); buf = lines.pop();
  <span class="kwd">for</span> (<span class="kwd">const</span> line <span class="kwd">of</span> lines) {
    <span class="kwd">if</span> (!line.startsWith(<span class="str">"data: "</span>)) <span class="kwd">continue</span>;
    <span class="kwd">const</span> data = line.slice(<span class="tok">6</span>);
    <span class="kwd">if</span> (data === <span class="str">"[DONE]"</span>) <span class="kwd">break</span>;        <span class="tok">// 水管关闭信号</span>
    <span class="kwd">const</span> j = JSON.parse(data);
    full += j.choices[<span class="tok">0</span>].delta.content || <span class="str">""</span>;
    <span class="tok">// 每收到一段就把 full 更新到界面（如 box.textContent = full）→ 打字机效果</span>
  }
}
console.log(full);</code></pre>
  ...
  -H <span class="str">"Authorization: Bearer sk-****"</span></code></pre>
      </div>
  ...
  <span class="str">"temperature"</span>: <span class="tok">0.7</span>,
  <span class="str">"stream"</span>: <span class="kwd">true</span>  <span class="tok">// 可选，流式返回</span>
}</code></pre>
        <h3>python</h3>
...
print(r.choices[<span class="tok">0</span>].message.content)</code></pre>
  ...
  -d <span class="str">'{"model": "ip-location", "ip": "8.8.8.8"}'</span></code></pre>
      ...
      <div class="api-card api-anchor">
        <h2><span class="api-method">GET</span><span class="api-endpoint">/assets/{key}</span></h2>
        <p class="desc">R2 媒体缓存访问。生成类接口（图片/音频/视频）产出的文件会写入缓存，可通过该路径直接访问，路径形如 <code>/assets/&#123;key&#125;</code>。</p>
        <pre><CopyBtn class="pre-copy" :text="codeApiAsset" /><code id="code-api-asset"><span class="kwd">curl</span> https://api.ltzy.top/assets/2026/08/abc123.png \
  -H <span class="str">"Authorization: Bearer sk-****"</span></code></pre>
      </div>
      ...
      <div class="api-card api-anchor">
        ...
        <pre><CopyBtn class="pre-copy" :text="codeApiErr" /><code id="code-api-err">{<span class="str">"error"</span>: {
  <span class="str">"message"</span>: <span class="str">"上游限流或额度受限，请稍后重试或更换模型（上游说明：…）"</span>,
  <span class="str">"type"</span>: <span class="str">"aqua_api_error"</span>,
  <span class="str">"code"</span>: <span class="str">"RATE_LIMITED"</span>,
  <span class="str">"status"</span>: <span class="tok">429</span>,
  <span class="str">"hint"</span>: <span class="str">"如需帮助：加入 QQ 频道 pd57362562（https://pd.qq.com/s/e4ktxw1b8）反馈，或访问官网 https://acu.ltzy.top"</span>,
  <span class="str">"help"</span>: { <span class="str">"site"</span>: <span class="str">"https://acu.ltzy.top"</span>, <span class="str">"docs"</span>: <span class="str">"https://acu.ltzy.top/#/api"</span>,
            <span class="str">"qq_guild"</span>: <span class="str">"pd57362562"</span>, <span class="str">"qq_guild_url"</span>: <span class="str">"https://pd.qq.com/s/e4ktxw1b8"</span>,
            <span class="str">"qq_guild_invite"</span>: <span class="str">"公告与支持见 QQ 频道（频道号 pd57362562）；注册登录后在官网控制台创建 API 密钥"</span>,
            <span class="str">"qq_group"</span>: <span class="tok">1103667832</span>, <span class="str">"qq_group2"</span>: <span class="tok">1006740220</span>, <span class="str">"qq_group2_url"</span>: <span class="str">"https://qm.qq.com/q/o8QDbza2Ge"</span> }
}}</code></pre>
      ...
      <div class="api-card api-anchor">
        ...
        <p class="desc" style="margin-top:8px;">示例：<code>curl https://你的网关/v1/tools/uuid -H "Authorization: Bearer sk-****"</code></p>
      </div>

      <!-- ===== 开放数据与社区玩法：状态大屏 / 竞技场 / 用量 ===== -->
      <div class="api-card api-anchor">
        <h2>开放数据与社区玩法 API</h2>
        ...
        <p class="desc" style="margin-top:8px;">试一把：<code>curl https://你的网关/v1/stats</code>（不带密钥也能访问）。</p>
      </div>
      ...
      <div class="api-card api-anchor">
        ...
        <details><summary><b>真的免费吗？有没有隐藏收费？</b></summary><p class="desc">除官方自营加速通道专线系列（<code>aqua/</code> 前缀 5 款，预充值按次计费，见上方「计费说明」章节）外，本站全部模型与工具均免费使用，无隐藏收费。个别通道有并发/额度保护（如部分通道日额度、专线通道并发限流），是为了让所有人都能公平使用，不是收费墙。免费模型不涉及任何余额与扣费，今后也不会收费。</p></details>
